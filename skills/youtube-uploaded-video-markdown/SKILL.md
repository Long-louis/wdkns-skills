...
description: Use when the user wants a cloud-friendly, full-depth YouTube Markdown note and has already uploaded the video file, subtitles, transcript, screenshots, metadata, thumbnail, or related artifacts to GPT Pro or another cloud agent. Use when the agent must create a detailed learning note, review digest, buying guide, or tutorial note from uploaded files only, without downloading from YouTube; Markdown is only the delivery format, not a reduction in content depth.
---

# YouTube Uploaded Video Markdown

Create a full-depth Markdown note from already uploaded YouTube video artifacts. Use this version for cloud agents that cannot call `yt-dlp`, fetch the full video, retrieve captions, or browse YouTube directly.

Markdown is only the carrier: do not simplify the knowledge, reasoning, evidence, figure coverage, or explanation depth compared with the PDF skills. Do not rely on the YouTube URL as the content source. Use the URL only as metadata/provenance when the user provides it.
...
- metadata text or JSON: title, channel, duration, description, chapter list, product list, links
...
1. Inventory uploaded files first. State which files will be used as video, subtitles/transcript, frames, thumbnail, metadata, and optional screenshots.
...
4. Never ask the cloud agent to download the YouTube URL, fetch captions, retrieve the full video, or browse YouTube for missing content.
...
- `assets/`: extracted frames, crops, uploaded screenshots, thumbnail/cover, generated charts, and diagrams used by the note
...
```markdown
![图 8：实测结果表的完整状态（subtitles.srt 12:31-12:46）](assets/frame-008-benchmark-table.jpg)
```
...
- Ground claims in uploaded subtitles, transcript text, frames, screenshots, chapters, metadata, or source filenames.

## Long Video Strategy

For long uploaded videos, do not rely on one monolithic pass.

- If the video is longer than 20 minutes, or subtitles contain more than 300 entries, split by uploaded chapters, coherent time windows, or subtitle ranges.
...
- uploaded metadata, title, description, thumbnail, chapters, and links
...
- like/subscribe prompts and routine channel logistics
...
- Do not fabricate missing screenshots, subtitles, prices, product names, measurements, timestamps, benchmark values, or links.
