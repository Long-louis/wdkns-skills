...
description: Use when the user wants a cloud-friendly Bilibili video summary skill and has already uploaded the video file, subtitles, transcript, screenshots, metadata, or related files to GPT Pro or another cloud agent, so the agent must create a Markdown learning note, review digest, or buying guide from uploaded artifacts without downloading from the Bilibili URL.
---

# Bilibili Uploaded Video Markdown

Create a Markdown note from already uploaded Bilibili video artifacts. Use this version for cloud agents that cannot fetch the whole video, call `yt-dlp`, access browser cookies, or download subtitles from Bilibili.

Do not rely on the Bilibili URL as the content source. The URL may be used only as metadata or provenance if the user provided it.
...
- metadata text: title, uploader, BV id, duration, description, chapter list, part list
...
1. Inventory uploaded files first. State which files will be used as video, subtitles/transcript, frames, cover, and metadata.
...
4. Never ask the cloud agent to download the Bilibili URL, fetch the full video, or retrieve missing CC subtitles from the platform.
5. For multi-part Bilibili uploads, process only the uploaded parts. If the part mapping is unclear, ask the user to identify the file order.
...
```markdown
![27 英寸 4K 推荐表](assets/frame-012-4k-monitor-table.jpg)
```
...
- Ground every important claim in uploaded subtitles, transcript text, frames, screenshots, or metadata.
...
- Separate `原视频观点` from `整理判断` when you add synthesis beyond the uploaded transcript.
- Skip greetings, danmaku, like/subscribe prompts, and channel logistics unless they materially affect the content.
- Do not fabricate missing screenshots, subtitles, prices, product names, or measurements.
